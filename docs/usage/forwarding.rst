...
The forwarding daemon is configured with a TOML file specifying the drivers to use
and the forwarding policies to apply.

.. code-block:: bash

   diaspora-ctl forward --config <config.toml>
...
The :code:`--config` argument is mandatory and points to a TOML configuration file.
...
The configuration file uses `TOML <https://toml.io>`_ format and consists of two sections:
a :code:`[drivers]` section defining the streaming backends, and one or more :code:`[[forward]]`
entries defining the forwarding policies.

Defining drivers
^^^^^^^^^^^^^^^^

Each driver is defined as a sub-table under :code:`[drivers]`. The table key becomes the
...
(using a Kafka driver). Any additional key-value pairs in the driver table are converted
to JSON and passed as metadata when creating the driver.

Defining forwarding policies
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Each :code:`[[forward]]` entry defines a policy that copies events from a source topic
to a destination topic. The :code:`from` and :code:`to` fields use the format
...
.. literalinclude:: ../_code/forward_local.toml
   :language: toml
...
.. code-block:: bash
   ...
   diaspora-ctl forward --config forward_local.toml
...
.. literalinclude:: ../_code/forward_cross_driver.toml
   :language: toml
...
.. literalinclude:: ../_code/forward_plugins.toml
   :language: toml
...
1. **Startup**: The TOML configuration is parsed and validated. All drivers are
